...
SortieBillesFusion <- function(Data, Type, dhs = 0.15, nom_grade1 = NA, long_grade1 = NA, diam_grade1 = NA, nom_grade2 = NA, long_grade2 = NA, diam_grade2 = NA,
  ...
  # On réajuste les noms de colonnes
  ...
  # On transpose les différentes coupes de Pétro pour obtenir une ligne par coupe
  Petro_transpo <- melt(Petro,
                     measure.vars = c("DER", "F1", "F2", "F3", "F4", "P"),
                     variable.name = "grade_bille")

  # Changer la valeur value de Pétro à vol_bille_dm3
  setnames(Petro_transpo, c("value"), c("vol_bille_dm3"))

  # Rejoindre les 2 tables ensemble
  Fusion <- rbind(Petro_transpo, Sybille, fill = TRUE)

  # Retirer les colonnes non-nécéssaire
  Fusion[, c("diam_fb_cm", "long_bille_pied","st_ha", "nbTi_ha", "type") := NULL]

  # Changer les valeurs de volume à 0
...
#result8 <- SortieBillesFusion(Result, Type = "DHP2015", dhs = 0.15, nom_grade1 = "sciage long", long_grade1 = 4, diam_grade1 = 8)
