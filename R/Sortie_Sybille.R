...
#' @importFrom OutilsDRF calcul_vol_bille
#'
#' @export
SortieSybille <- function(Data, dhs = 0.15, nom_grade1 = NA, long_grade1 = NA, diam_grade1 = NA, nom_grade2 = NA, long_grade2 = NA, diam_grade2 = NA,
  ...
  # Chargement de PropEPX pour avoir accès à la table
  #PropEPX <- data.table::fread("data_raw/PropEPX.csv")

  # Faire une copie de PropEPX
  ...
  setnames(Data, c("Veg_Pot", "PlacetteID", "DHPcm", "Altitude", "hauteur_pred", "origTreeID", "EssenceFinale", "Cl_Drai"),
  ...
  Data[, cl_drai := str_sub(cl_drai, 1, 1)]
  ...
  Data_treated <- Data[, .(essence, id_pe, no_arbre, sdom_bio, cl_drai, veg_pot, DHP_Ae, HT_REELLE_M, HAUTEUR_M, nbTi_ha, st_ha, ALTITUDE)]
...
#SortieSybille(Result, nom_grade1 = "pate", long_grade1 = 8, diam_grade1 = 4)
